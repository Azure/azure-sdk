...
- `Replace` - This field is used the store the package name of the newer package that replaces the older package. The value should be the exact name of the package (for java it should be `groupdid\artifactid`). If there are multiple hen they should be separated by a comma (`,`). If a package is marked as `deprecated`, this field must have a valid value.
