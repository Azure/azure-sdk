...
function Write-Latest-Versions($lang)
{
  ...
  if($lang -eq 'dotnet')
  {
    ...
    $linkTemplates = GetLinkTemplates $lang
    ...
    foreach ($pkg in $packageList){
      if(($pkg.Support -eq "deprecated"))
      {
        ...
        # 2) Use the package status query service to get the package index
        $packageIndex = "$registrationUrl/$packageName/index.json".ToLowerInvariant()
        ...
        if ( -not ($metadata.catalogEntry | Select-Object -ExpandProperty deprecation -First 1 -ErrorAction SilentlyContinue))
        {
          ...
          $deprecationMsg += " Refer to our deprecation policy (https://azure.github.io/azure-sdk/policies_support.html) for more details."
          $headers = @{
            "X-NuGet-ApiKey" = "$(azure-sdk-nuget-pat)"
