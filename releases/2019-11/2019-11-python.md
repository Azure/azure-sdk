...
- `azure.storage.file` is now `azure.storage.fileshare`.
  - The package is renamed from `azure-storage-file` to `azure-storage-file-share`.
  - `FileServiceClient` is now `ShareServiceClient`.
  - `DirectoryClient` is now `ShareDirectoryClient`.
  - `FileClient` is now `ShareFileClient`.
- Replaced iterable behavior with chunk based behavior for downloading blobs.
- `StorageStreamDownloader` now has a similar interface as Stream with `readinto()` and `readall()` methods.
