...
{% include requirement/MUST id="golang-lro-poller-def" %} provide the following methods on a `<Resource>Poller` type: `Done()`, `ID()`, `Response()`, `Poll()`, and `Wait()`.
...
type WidgetPoller interface {
	...
	Poll(context.Context) (*Widget, error)
	...
	Wait(ctx context.Context, pollingInterval time.Duration) (*Widget, error)
	...
	RegisterProgressReporter(reporter func(string))
}
```
...
func (c *WidgetClient) BeginCreate(ctx context.Context, name string) (*WidgetPoller, error) {
	// ...
}
```
...
// The ID must come from a previous call to WidgetPoller.ID().
func (c *WidgetClient) ResumeWidgetPoller(id string) *WidgetPoller {
	// ...
}
```
...
poller, err := client.BeginCreate(context.Background(), "blue_widget")
if err != nil {
	// handle error...
}
...
poller := client.ResumeWidgetPoller("operation ID from external process")
...
type WidgetClient interface {
	BeginCreate(ctx context.Context, options factory.WidgetOptions) (*factory.WidgetPoller, error)
