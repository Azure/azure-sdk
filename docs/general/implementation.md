...
### Validation in multi-api-version clients

A service client that supports multiple service versions (see [Service API Versions]) may expose parameters, models, and methods that are not supported by all service versions. While the above guidance generally applies for these situations, some special provisions are appropriate. An example is a service that adds support for conditional requests in a new service version. To support this, the client will expose parameters for the conditional request headers ("if-match", "if-unmodified-since", etc), but if these are used with a client configured with an earlier service version the result is potentially dangerous. If the service does not check for and reject requests that specify these headers with the earlier api-version, if may perform operations that violate the condition.

{% include requirement/MUST id="general-params-apiversion-9" %} validate that query and header parameters passed to a method are defined for the api-version specified when the client was constructed and return an error if the validation fails.
...
[Service API versions]: {{ site.baseurl }}{% link docs/general/design.md#service-api-versions %}
