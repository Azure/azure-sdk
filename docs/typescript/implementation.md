...
{% include requirement/SHOULD %} use the [TypeScript client library template].
...
{% include requirement/MUST %} use the `@azure/core-configuration` package.  The `@azure/core-configuration` package implements the general guidelines for configuration; specifically:
...
{% include requirement/MUST %} use the HTTP pipeline component within `@azure/core-http` package for communicating to service REST endpoints.
...
{% include requirement/MUST %} use ECMAScript built-in error types for validation failures when appropriate. Specifically,
...
{% include requirement/SHOULD %} coerce incorrect types into an appropriate type, if possible. JavaScript users expect some amount of fuzziness with parameters as the standard library will coerce types if possible. TypeScript users should get pedantic types as they've opted in to types and expect errors.
...
{% include requirement/MUST %} use the `debug` module to log to stderr or the browser console.
...
{% include requirement/MUST %} prefix channel names with `azure:<service-name>`.

{% include requirement/MUST %} create log channels for the following log levels with the following channel name suffixes:
...
{% include requirement/MAY %} have additional log channels, for example, to log from separate components. However, these channels MUST still provide the three log levels from above for each subchannel.

{% include requirement/MUST %} expose all log channels as top-level exports of your package, allowing the consumer to configure how the logging happens and integrate with 3rd-party loggers.

{% include requirement/MUST %} use the Error channel for failures that the application is unlikely to recover from (out of memory, etc.).

{% include requirement/MUST %} use the Warning channel when a function fails to perform its intended task. This generally means that the function will raise an exception.  Do not include occurrences of self-healing events (for example, when a request will be automatically retried).

{% include requirement/MUST %} use the Info channel when a function operates normally.

{% include requirement/MUST %} use the Verbose channel for detailed troubleshooting scenarios. This is primarily intended for developers or system administrators to diagnose specific failures.

{% include requirement/MUSTNOT %} send sensitive information in channels other than Verbose. For example, remove account keys when logging headers.

{% include requirement/MUST %} log request line, response line, and headers on the Info channel.

{% include requirement/MUST %} log to the Info channel if a service call is cancelled.

{% include requirement/MUST %} log exceptions thrown to the Warning channel. Additionally, send stack trace information to the Verbose channel.
...
{% include requirement/MUST %} take an option named `parentSpanId` for all asynchronous operations.
...
{% include requirement/MUSTNOT %} depend any other packages within the client library distribution package. Dependencies are thoroughly vetted through architecture review.  Build dependencies, by contrast, are acceptable and commonly used.
...
{% include requirement/MUSTNOT %} depend on concrete logging, dependency injection, or configuration technologies (except as implemented in the Azure Core library).

{% include requirement/SHOULDNOT %} take dependencies on tiny libraries as the cost of many small libraries adds up over time. Larger dependencies are subject to approval.

{% include requirement/SHOULDNOT %} depend directly on polyfills or other libraries that modify global scope. If developers using older runtimes need to polyfill some capability, the package install and usage instructions (in the `README`) should indicate this dependency.
...
{% include requirement/SHOULD %} use [Mocha](https://mochajs.org/) and [Karma](http://karma-runner.github.io/4.0/index.html) as these tools support build pipelines and work in browsers and node.
...
{% include requirement/MUSTNOT %} include new features in a patch release.

{% include requirement/MUST %} increment the major or minor version when adding support for a service API version, or add a backwards-compatible feature.

{% include requirement/MUSTNOT %} make breaking changes.  If a breaking change is absolutely required, then you **MUST** engage with the [Architecture Board] prior to making the change.  If a breaking change is approved, increment the major version. 

{% include requirement/SHOULD %} increment the major version when making large feature changes.

{% include requirement/MUST %} provide the ability to call a specific supported version of the service API.
...
{% include requirement/MUST %} version with [semver](https://semver.org/). Deprecated features and flags must offer an alternate stable or beta path for developers.
...
{% include requirement/MUST %} have npm package ownership set to either the Azure or Microsoft organizations.
...
{% include requirement/MUST %} follow these conventions where applicable.

{% include requirement/MAY %} include other files.

{% include requirement/MUSTNOT %} use `.npmignore` files to control which files are included in the package. All files must be added to the package explicitly using the `package.json` files key.
...
{% include requirement/MUST %} set `name` to `@azure/<name>`, where `<name>` is the name of the service. Package names are kebab-case: all lowercase with words joined by dashes.

{% include requirement/MUST %} set `homepage` to a URL pointing to your library's readme inside the git repo. Since the repository link goes to the monorepo, this link exists to serve as an easier way to reach the actual package's source.  

{% include requirement/MUST %} set `bugs` to an object with a `url` key pointing to your library's issue tracker: https://github.com/Azure/azure-sdk-for-js/issues.

{% include requirement/MUST %} set `repository` to the JS SDK monorepo - `github:Azure/azure-sdk-for-js`. Use of the `github:user/repo` short-hand is recommended.

{% include requirement/MUST %} set `description` to a useful but terse description of your library. The description is used and shown when searching for packages on [npmjs.org](https://npmjs.org).

{% include requirement/MUST %} set `keywords` to an array that includes at least the entries "Azure" and "cloud". It must also contain at least the name of your service. It should contain other entries relevant to your SDK.

{% include requirement/MUST %} set `author` to `"Microsoft Corporation"`.

{% include requirement/MUST %} set `sideEffects` to `false`. Side effecting libraries must be explicitly approved during design review. The `sideEffects` field is used by [Webpack](https://webpack.js.org) and potentially other tools as an indicator of how aggressively the package can be optimized.
...
{% include requirement/MUST %} set `main` to point to either a CommonJS or a UMD module. Main is the entry point of your application for Node users.

{% include requirement/MUSTNOT %} set `main` to include any ES6+ syntax.

{% include requirement/MUST %} set `module` to the ES6 module entrypoint of your application.
...
{% include requirement/MUST %} include a file map in the `browser` object if your library supports the browser.  The file map must include the `main` entry, and map it to the corresponding (unminified) browser code.
...
{% include requirement/MUST %} set `engine` to the versions of Node your library supports. See [#ts-supported-node-versions] for Node support requirements.

{% include requirement/MUST %} set `scripts` to an object with the following scripts:
...
{% include requirement/MUSTNOT %} depend on shell scripts to build or test the package.  Shell scripts need to be platform-specific.  Include a `script` for any task required during development of your package.

{% include requirement/MUST %} set `files` to an array containing paths of your package contents. Setting this field prevents extraneous files from ending up in your package by being explicit about which files you ship to npm. 

{% include requirement/MUST %} set `types` to point to the TypeScript type declarations for your library's public surface area, usually `"./types/index.d.ts"`.

{% include requirement/MUST %} set `license` to "MIT".
...
{% include requirement/MUST %} include the source code in your package.  Include your source directory in the `files` array in your `package.json`.
...
{% include requirement/MUST %} include a CommonJS (CJS) build in your package if you intend to support Node.

{% include requirement/SHOULD %} distribute your package as a UMD module if you intend to support browsers.
...
{% include requirement/MUST %} flatten the CommonJS or UMD module.  [Rollup](https://rollupjs.org) is recommended for producing a flattened module.
...
{% include requirement/MUST %} include an ECMAScript Module (ESM) build in your package.

{% include requirement/MUSTNOT %} flatten the ESM build.
...
{% include requirement/SHOULD %} provide a browser build in UMD format for your library.

{% include requirement/MUST %} name the UMD global according to the [namespace guidelines].

{% include requirement/MUST %} provide both minified and non-minified versions, both with source mapping.

{% include requirement/MUST %} lace browser builds in a top level `browser` folder. The name of the file should be the service name. Append `.min` to the name of minified files. For example, Storage Blob should have `storage-blob.min.js` and `storage-blob.js` under `./browser`.

### Modules

{% include requirement/MUST %} have named exports at the top level

{% include requirement/MUSTNOT %} have a default export at the top level
