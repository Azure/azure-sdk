...
* The SDK should follow the general design guidelines and conventions for iOS libraries written in Swift. It should feel natural to a Swift developer.
...
{% include requirement/MUST id="ios-service-client-module" %} place all service clients and supporting types in a single module.
...
{% include requirement/MAY id="ios-service-client-module-subpackage" %} break a single library into sub-packages where appropriate, in which case you must include a feature suffix (for example, `AzureStorageBlob`) in the module name.
...
{% include requirement/MUST id="ios-service-client-feature-support" %} support only those features provided by the Azure service that a mobile app would access. While gaps in functionality do cause frustration, binary size in the mobile ecosystem is of a bigger concern.

{% include requirement/MUST id="ios-service-client-mobile-consistency" %} be consistent with other mobile platforms in feature support (for example, Android).
...
{% include requirement/MUST id="ios-client-options-parameter" %} accept all optional initializer arguments via an [options struct](#options-struct) provided as a single, final parameter named `options`. Do not accept individual initializer options as separate parameters.
...
class ConfigurationClient {
    public init(endpoint: String, credential: String? = nil, options: ConfigurationClientOptions? = nil) {
        ...
    }

    public init(connectionString: String, options: ConfigurationClientOptions? = nil) {
        ...
    }
}
{% endhighlight %}
...
{% include requirement/MUST id="ios-versioning-select-service-api" %} provide an enum of supported service API versions that can be supplied via the [options struct](#options-struct) when initializing the service client, as shown below:
...
{% include requirement/MUST id="ios-network-callback-final" %} accept a callback as the final parameter for service methods, allowing developers to take advantage of Swift's trailing closure syntax.

{% include requirement/SHOULD id="ios-network-callback-single" %} accept only a single callback for any given service method. Providing a method that accepts multiple callbacks leads to unnecessarily cluttered code, and only the final callback parameter can make use of the trailing closure syntax.
...
{% include requirement/MAY id="ios-network-delegate" %} provide a delegate protocol that the developer can conform to instead of a callback parameter for service methods where use of the delegate would improve clarity and/or reduce clutter. For such methods, you may either accept the delegate as the final parameter or return an object to which the delegate can be attached.
...
{% include requirement/MUST id="ios-service-client-verb" %} use the verb by itself as the method name when the name of the first parameter makes it obvious what object the action will apply to. For example, prefer `configurationClient.get(key: 'foo', completionHandler: ...)` rather than `configurationClient.getKey(name: 'foo', completionHandler: ...)` or `configurationClient.getKey('foo', completionHandler: ...)`.
...
{% include requirement/MUSTNOT id="ios-async-suffix" %} use the suffix `Async` for service methods or other methods that do operations asynchronously.
