...
After the fix has been released you need to decide if the changes in the hotfix branch need to be merged back into `master`, if you cherry-picked the changes from master then you should not need to merge them back. If they do need to be merged then from a local `master` branch run the merge `git merge hotfix/<hotfix name>` and handle the merge conflicts. If nothing else your version numbers will conflict and you should accept the versions that are already in `master` over what came from the hotfix branch.

Once the hotfix has been released and any changes merged back to `master`, if necessary, then you should now delete the `hotfix/<hotfix name>` branch, it can always be recreated in the future from the last release tag if needed. 
