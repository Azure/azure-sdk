...
* **DO NOT** use upper case in the anchor tags like `#Samples`. Use upper cases in anchor tags do not always direct to the right link when we consume the content in other places, such as docs, samples, package managers, etc. For this reason the guidance is to use lower case anchor tags in all our MD files in the repo as this will help ensure that links work in other contexts. For example use `https://github.com/Azure/azure-sdk-for-java#client-new-releases` instead of `https://github.com/Azure/azure-sdk-for-java#Client-New-Releases`
