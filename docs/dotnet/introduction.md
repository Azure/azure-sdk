...
{% include requirement/MUST %} follow the official [.NET Framework Design Guidelines]. 
...
{% include requirement/MUST %} place at least one service client in the root namespace of their corresponding component.

{% include requirement/MUST %} make service clients classes (reference types), not structs (value types).

{% include requirement/MUST %} see [Namespace Naming](#dotnet-namespace-naming) guidelines for how to choose the namespace for the client types.

### Service Client Constructors {#dotnet-client-ctor}

{% include requirement/MUST %} provide a minimal constructor that takes only the parameters required to connect to the service.  
...
{% include requirement/MUSTNOT %} use default parameters in the _hello world_ constructor.

{% include requirement/MUST %} provide constructor overloads that allow specifying additional options such as credentials, a custom HTTP pipeline, or advanced configuration.
...
{% include requirement/MUST %} provide protected parameterless constructor for mocking.
...
{% include requirement/MUST %} provide both asynchronous and synchronous variants for all service methods.

{% include requirement/MUST %} ensure that the names of the asynchronous and the synchronous variants differ only by the _Async_ suffix.

{% include requirement/MUST %} ensure all service methods, both asynchronous and synchronous, take an optional `CancellationToken` parameter called _cancellationToken_.

The token should be passed to all I/O calls.  Don't check the token manually.

{% include requirement/MUST %} make service methods virtual.

Virtual methods are used to support mocking. See [Supporting Mocking](#dotnet-mocking) for details.

{% include requirement/MUST %} return `Response<T>` or `Response` from synchronous methods.

`T` represents the content of the response. For more information, see [Service Method Return Types](#dotnet-method-return).

{% include requirement/MUST %} return `Task<Response<T>>` or `Task<Response>` from asynchronous methods that make network requests.
...
{% include requirement/MUST %} be thread-safe. All public members of the client type must be safe to call from multiple threads concurrently.
...
{% include requirement/MUST %} use one of the following return types to represent an unstructured payload:

* `System.IO.Stream` - for large payloads
* `byte[]` - for small payloads
* `ReadOnlyMemory<byte>` - for slices of small payloads

{% include requirement/MUST %} return a _model type_ if the content has a schema and can be deserialized.
...
{% include requirement/MUST %} ensure model public properties are get-only if they aren't intended to be changed by the user.
...
{% include requirement/MUST %} ensure model types are structs if they're small, and classes if they're large, per [.NET Framework Design Guidelines](https://docs.microsoft.com/en-us/dotnet/standard/design-guidelines/choosing-between-class-and-struct).

{% include requirement/MUST %} implement basic data type interfaces on model types, per .NET Framework Design Guidelines. 
...
{% include requirement/SHOULD %} apply the `[EditorBrowsable(EditorBrowsableState.Never)]` attribute to methods that the user isn't meant to call.  
...
{% include requirement/MUST %} ensure all model types can be used in mocks.
...
{% include requirement/MUST %} return `IEnumerable<Response<T>>` or ``IAsyncEnumerable<Response<T>>` from service methods that return a collection of items.
...
{% include requirement/SHOULD %} expose raw paging APIs through custom enumerable types with a `ByPage()` method. 
...
{% include requirement/MUSTNOT %} validate service parameters.
...
{% include requirement/MUST %} test the developer experience when invalid service parameters are passed in. Ensure clear error messages are generated by the client. If the developer experience is inadequate, work with the service team to correct the problem.
...
{% include requirement/MUST %} name all methods that start an LRO with the `Start` prefix.

{% include requirement/SHOULD %} provide an overload of the method that starts the LRO taking operationId parameter.
...
{% include requirement/MUST %} make all service methods virtual.

{% include requirement/MUST %} provide protected parameterless constructor for mocking.

{% include requirement/MUST %} provide factory or builder for constructing model graphs returned from virtual service methods. 
...
{% include requirement/SHOULD %} use credential types provided in the `Azure.Core` package.

Currently, `Azure.Core` provides `TokenCredential` for OAuth style tokens, including MSI credentials.

{% include requirement/MUST %} support changing credentials without having to create a new client instance.

Credentials passed to the constructors must be read before every request (for example, by calling `TokenCredential.GetToken()`).

{% include requirement/MUST %} contact [adparch] if you want to add a new credential type.

{% include requirement/MAY %} offer a way to create credentials from a connection string only if the service offers a connection string via the Azure portal.
...
{% include requirement/MUST %} adhere to the following scheme when choosing a namespace: `Azure.<group>.<service>[.<feature>]`
...
{% include requirement/MUST %} register all namespaces with [adparch].

{% include requirement/MUSTNOT %} place APIs in the second-level namespace (directly under the `Azure` namespace).

{% include requirement/SHOULD %} consider placing model types in a `.Models` namespace if number of model types is or might become large.  
...
{% include requirement/MUST %} throw ```ResponseFailedException``` or its subtype when a service method fails with non-success status code. 
...
{% include requirement/SHOULD %} use `ResponseExceptionExtensions` to create `RequestFailedException` instances.
...
{% include requirement/MUST %} use `ResponseFailedException` or one of its subtypes where possible.
...
{% include requirement/MUST %} package all components as NuGet packages.
...
{% include requirement/MUST %} name the package based on the name of the main namespace of the component.
...
{% include requirement/SHOULD %} place small related components that evolve together in a single NuGet package.

{% include requirement/MUST %} build all libraries for [.NET Standard 2.0].
...
{% include requirement/SHOULD %} minimize dependencies outside of the .NET Standard and `Azure.Core` packages.

{% include requirement/MUSTNOT %} depend on any NuGet package except the following packages:
...
{% include requirement/MUST %} be 100% backwards compatible with older versions of the same package.
...
{% include requirement/MUST %} call the highest supported service API version by default.

{% include requirement/MUST %} allow the consumer to explicitly select a supported service API version when instantiating the service client.
...
{% include requirement/MUST %} introduce a new package (with new assembly names, new namespace names, and new type names) if you must do an API breaking change.
...
{% include requirement/MUSTNOT %} force consumers to test service API versions to check support for a feature.  Use the _tester-doer_ .NET pattern to implement feature flags, or use `Nullable<T>`.
...
{% include requirement/MUST %} use _MAJOR_._MINOR_._PATCH_ format for the version of the library dll and the NuGet package.

Use _-preview_._N_ suffix for preview package versions. For example, _1.0.0-preview.2_.

{% include requirement/MUST %} change the version number of the client library when **ANYTHING** changes in the client library.

{% include requirement/MUST %} increment the patch version when fixing a bug.

{% include requirement/MUSTNOT %} include new APIs in a patch release.

{% include requirement/MUST %} increment the major or minor version when adding support for a service API version.

{% include requirement/MUST %} increment the major or minor version when adding a new method to the public API.

{% include requirement/SHOULD %} increment the major version when making large feature changes.

### Documentation {#dotnet-documentation}

{% include requirement/MUST %} document every exposed (public or protected) type and member within your library's code.
...
{% include requirement/MUST %} name subclasses of ```HttpPipelineOptions``` by adding _Client_ suffix to the name of the client type the options subclass is configuring.
...
{% include requirement/MUSTNOT %} have a default constructor on the options type.
...
{% include requirement/MUST %} inherit from `HttpPipelinePolicy` if the policy implementation calls asynchronous APIs.
...
{% include requirement/MUST %} inherit from `SynchronousHttpPipelinePolicy` if the policy implementation calls only synchronous APIs.
...
{% include requirement/MUST %} be thread-safe. The `ProcessAsync` and `Process` methods must be safe to invoke from multiple threads concurrently.

`HttpPipelineMessage`, `Request`, and `Response` don't have to be thread-safe.

### JSON {#dotnet-usage-json}

Use [`System.Text.Json`](https://www.nuget.org/packages/System.Text.Json) package to write and read JSON content.

{% include requirement/SHOULD %} use `Utf8JsonWriter` to write JSON payloads:
...
{% include requirement/SHOULD %} use `JsonDocument` to read JSON payloads:
...
{% include requirement/SHOULD %} consider using `Utf8JsonReader` to read JSON payloads.

`Utf8JsonReader` is faster than `JsonDocument` but much less convenient to use.

{% include requirement/MUST %} make your serialization and deserialization code version resilient.

Optional JSON properties should be deserialized into nullable model properties.

### Primitive Types

{% include requirement/MUST %} use `Azure.ETag` to represent ETags.

The `Azure.ETag` type is located in `Azure.Core` package.

{% include requirement/MUST %} use `System.Uri` to represent URIs.

# Repository Guidelines {#dotnet-repository}

{% include requirement/MUST %} locate all source code and README in the [azure/azure-sdk-for-net] GitHub repository.

{% include requirement/MUST %} follow Azure SDK engineering systems guidelines for working in the [azure/azure-sdk-for-net] GitHub repository.

## README {#dotnet-repository-readme}

{% include requirement/MUST %} have a README.md file in the component root folder.
...
{% include requirement/MUST %} optimize the `README.md` for the consumer of the client library.
...
{% include requirement/MUST %} have usage samples in `samples` subdirectory of main library directory.
...
{% include requirement/MUST %} have a sample file called `S1_HelloWorld.cs`. All other samples are ordered from simplest to most complex using the `S<number>_` prefix.

{% include requirement/MUST %} use synchronous APIs in the `S1_HelloWorld.cs` sample.  Add a second sample (named `S2_HelloWorldAsync.cs`) that does the same thing as `S1_HelloWorld.cs` using asynchronous code.

{% include requirement/MUST %} make sure all the samples build and run as part of the CI process.
...
{% include requirement/SHOULDNOT %} have many types in the main namespace. Number of types is directly proportional to the perceived complexity of a library.

{% include requirement/MUSTNOT %} use abstractions unless the Azure SDK both returns and consumes the abstraction.  An abstraction is either an interface or abstract class.

{% include requirement/MUSTNOT %} use interfaces if you can use abstract classes. The only reasons to use an interface are: a) you need to “multiple-inherit”, b) you want structs to implement an abstraction.

{% include requirement/MUSTNOT %} use generic words and terms for type names.  For example, do not use names like `OperationResponse` or `DataCollection`.

{% include requirement/SHOULDNOT %} use parameter types where it’s not clear what valid values are supported.  For example, do not use strings but only accept certain values in the string.
