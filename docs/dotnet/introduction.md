...
If your client library is built by the Azure SDK engineering systems, all packaging requirements will be met automatically. Follow the [.NET packaging guidelines](https://docs.microsoft.com/en-us/dotnet/standard/library-guidance/nuget) if you're self-publishing. One deviation from the standard package guidelines for Microsoft owned packages is we need to publish symbols to the Microsoft symbol server because we need to support both windows (for windows dump diagnostics) and portable (for x-platform debugging) pdb formats, whereas the Nuget symbol server only supports portable pdbs. 
