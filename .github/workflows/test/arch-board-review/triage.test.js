...
`
    ...
    const checkboxLines = [
        ...
        'C++',
        'Rust'
    ].map((language) => `- [${languages.includes(language) ? 'x' : ' '}] ${language}`).join('\n');
...
`
        ...
        expect(github.rest.issues.addLabels).toHaveBeenNthCalledWith(1, expect.objectContaining({ labels: ['Java'] }));
        expect(github.rest.issues.addLabels).toHaveBeenNthCalledWith(2, expect.objectContaining({ labels: ['needs-info'] }));
    ...
    it('applies ready-for-review when all artifacts validate', async () => {
        ...
        expect(github.rest.issues.addLabels).toHaveBeenNthCalledWith(1, expect.objectContaining({ labels: ['Java'] }));
        expect(github.rest.issues.addLabels).toHaveBeenNthCalledWith(2, expect.objectContaining({ labels: ['ready-for-review'] }));
        expect(github.rest.issues.removeLabel).toHaveBeenCalledWith(expect.objectContaining({ name: 'needs-info' }));
        expect(github.rest.issues.createComment).toHaveBeenCalledWith(expect.objectContaining({
...
`
        ...
        expect(analysis.missing).toEqual(expect.arrayContaining([
