 ...
 */
const ALLOWED_HOSTS = ["apiview.dev", "github.com", "learn.microsoft.com", "azure.github.io"];
...
function hasApiViewReviewId(url) {
  try {
    ...
    return (
      /\/review\/[a-z0-9-]+/i.test(parsed.pathname) ||
...
async function validateUrl(url, { fetchImpl = globalThis.fetch, timeoutMs = 10000 } = {}) {
  ...
  const headSucceeded = await tryFetch(fetchImpl, url, {
    method: "HEAD",
    redirect: "follow",
    signal,
  });
  ...
  const getSucceeded = await tryFetch(fetchImpl, url, {
    method: "GET",
    redirect: "follow",
    signal,
  });
  return getSucceeded ? { valid: true, reason: null } : { valid: false, reason: "unreachable" };
}

export { ALLOWED_HOSTS, hasApiViewReviewId, isAllowedHost, validateUrl };
